...
ttWait:"Buscando LIVE…",ttOff:"Ese usuario no está en vivo ahora.",
...
ttWait:"Looking for LIVE…",ttOff:"That user is not live right now.",
...
function initials(name){const p=String(name||"?").replace(/[^\p{L}\p{N}]+/gu," ").trim().split(/\s+/);return ((p[0]||"?")[0]+(p[1]?p[1][0]:"")).toUpperCase();}
...
function startDemo(){stopDemo();demoOn=true;$("demo").checked=true;let i=0;const tick=()=>{const item=demoScript[i%demoScript.length];i++;addMsg({...item,displayName:item.display,ts:Date.now(),source:"demo"});};tick();demoTimer=setInterval(tick,2800);refreshPill();}
function stopDemo(){demoOn=false;$("demo").checked=false;if(demoTimer)clearInterval(demoTimer);demoTimer=null;refreshPill();}
function parseTwitch(raw){let tags={},rest=raw;if(rest.startsWith("@")){const sp=rest.indexOf(" ");rest.slice(1,sp).split(";").forEach(p=>{const [k,v]=p.split("=");tags[k]=v||"";});rest=rest.slice(sp+1);}const prefixEnd=rest.startsWith(":")?rest.indexOf(" "):-1;const prefix=prefixEnd>0?rest.slice(1,prefixEnd):"";const after=prefixEnd>0?rest.slice(prefixEnd+1):rest;const cmdEnd=after.indexOf(" :");const head=cmdEnd>=0?after.slice(0,cmdEnd):after;const text=cmdEnd>=0?after.slice(cmdEnd+2):"";const parts=head.split(" ");return {tags,nick:prefix.split("!")[0],cmd:parts[0],args:parts.slice(1),text};}
...
function save(){localStorage.setItem("voxlive",JSON.stringify({lang,twitch:$("twitch").value,tiktok:$("tiktok").value,ttsOn,readName,rate,selectedVoice}));}
...
$("es").onclick=()=>{lang="es";$("es").classList.add("on");$("en").classList.remove("on");applyLang();save();};
$("en").onclick=()=>{lang="en";$("en").classList.add("on");$("es").classList.remove("on");applyLang();save();};
$("tts").onchange=e=>{ttsOn=e.target.checked;if(!ttsOn){speechSynthesis.cancel();queue.length=0;speaking=null;kick();}save();};
$("readname").onchange=e=>{readName=e.target.checked;save();};
$("voice").onchange=e=>{selectedVoice=e.target.value;save();};
$("rate").oninput=e=>{rate=Number(e.target.value);$("rate-v").textContent=rate.toFixed(1)+"×";save();};
$("demo").onchange=e=>{e.target.checked?startDemo():stopDemo();};
$("test").onclick=()=>{const u=new SpeechSynthesisUtterance(t().speak);u.lang=lang==="es"?"es-US":"en-US";u.rate=rate;const pref=pickVoice();if(pref)u.voice=pref;speechSynthesis.speak(u);};
$("skip").onclick=()=>{speechSynthesis.cancel();speaking=null;kick();};
$("pause").onclick=()=>{paused=!paused;$("pause").textContent=paused?t().resume:t().pause;if(paused)speechSynthesis.pause();else{speechSynthesis.resume();kick();}};
$("stage").onclick=()=>{document.body.classList.toggle("stage");applyLang();};
$("twitch-btn").onclick=()=>{if(twitchOn)stopTwitch();else{stopDemo();startTwitch($("twitch").value);}applyLang();save();};
$("tiktok-btn").onclick=()=>{if(tiktokOn)stopTikTok();else startTikTok($("tiktok").value);applyLang();save();};
